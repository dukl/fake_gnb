...
web::json::value StatusInfo::toJson() const
{
    ...
    val[utility::conversions::to_string_t("resourceStatus")] = ModelBase::toJson(m_ResourceStatus);
    if(m_CauseIsSet)
    {
...
void StatusInfo::fromJson(const web::json::value& val)
{
    ...
    newResourceStatus->fromJson(val.at(utility::conversions::to_string_t("resourceStatus")));
...
void StatusInfo::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
    ...
    m_ResourceStatus->toMultipart(multipart, utility::conversions::to_string_t("resourceStatus."));
    if(m_CauseIsSet)
    {
        if (m_Cause.get())
        {
...
void StatusInfo::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
    ...
    std::shared_ptr<ResourceStatus> newResourceStatus(new ResourceStatus());
    newResourceStatus->fromMultiPart(multipart, utility::conversions::to_string_t("resourceStatus."));
    setResourceStatus( newResourceStatus );
...
void StatusInfo::setResourceStatus(const std::shared_ptr<ResourceStatus>& value)
{
