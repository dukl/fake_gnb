...
void Tai::fromJson(const web::json::value& val)
{
    ...
    setPlmnId( newPlmnId );
    setTac(ModelBase::stringFromJson(val.at(utility::conversions::to_string_t("tac"))));
}
...
void Tai::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
    ...
    setPlmnId( newPlmnId );
    setTac(ModelBase::stringFromHttpContent(multipart->getContent(utility::conversions::to_string_t("tac"))));
}
