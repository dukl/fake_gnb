...
{
    setSst(ModelBase::int32_tFromJson(val.at(utility::conversions::to_string_t("sst"))));
...
void Snssai::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
    ...
    setSst(ModelBase::int32_tFromHttpContent(multipart->getContent(utility::conversions::to_string_t("sst"))));
    if(multipart->hasContent(utility::conversions::to_string_t("sd")))
    {
        setSd(ModelBase::stringFromHttpContent(multipart->getContent(utility::conversions::to_string_t("sd"))));
    }
}
